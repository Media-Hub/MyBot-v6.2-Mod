...
Func MainSuperXPHandler()
	...
	While $canGainXP = True
		...
		If OpenGoblinPicnic() = False Then Return False
		If $RunState = False Then Return
...
Func ActivateHeroesByDelay($hBdTimer)
	...
	If $tDiff >= $QueenDelay And $QueenDelay <> 0 And $ActivatedHeroes[0] = False And $Queen <> -1 And $ichkSXAQ <> $HERO_NOHERO Then
		If $DebugSX = 1 Then SetLog("SX|Activating Queen Ability After " & $tDiff & "/" & $QueenDelay & " ms(s)")
	...
	If $tDiff >= $WardenDelay And $WardenDelay <> 0 And $ActivatedHeroes[1] = False And $Warden <> -1 And $ichkSXGW <> $HERO_NOHERO Then
		If $DebugSX = 1 Then SetLog("SX|Activating Warden Ability After " & $tDiff & "/" & $WardenDelay & " ms(s)")
	...
	If $tDiff >= $KingDelay And $KingDelay <> 0 And $ActivatedHeroes[2] = False And $King <> -1 And $ichkSXBK <> $HERO_NOHERO Then
		If $DebugSX = 1 Then SetLog("SX|Activating King Ability After " & $tDiff & "/" & $KingDelay & " ms(s)")
...
Func IsInAttackSuperXP()
	...
	If _ColorCheck(_GetPixelColor(88, 19, True), Hex(0xFCFFFF, 6), 20) And _ColorCheck(_GetPixelColor(59, 25, True), Hex(0xFFFAFA, 6), 20) Then Return True
...
Func WaitForNoClouds()
	...
	Local $i = 0
	ForceCaptureRegion()
	;While _ColorCheck(_GetPixelColor(20, 675, True), Hex(0x040D0B, 6), 25) = False Or _ColorCheck(_GetPixelColor(785, 600, True), Hex(0x090F09, 6), 25) = False ; wait for clouds to be gone
...
Func IsInGoblinPicnic($Retry = True, $maxRetry = 30, $timeBetweenEachRet = 300)
	...
	While $Found = False
		...
		$result = multiMatchesPixelOnly($directory, 0, "FV", "", "", 0, 1000, 48, 12, 110, 30)
		If $DebugSX = 1 Then SetLog("SX|IGP|$result=" & $result)
		$Found = Not (StringLen($result) < 3 And StringInStr($result, "|"))
...
Func DragToGoblinPicnic()
	...
	Switch $posInSinglePlayer
		Case "END"
			...
			While $rIsGoblinPicnicFound = False
				...
				If $rIsGoblinPicnicFound = True Then ExitLoop
				$Counter += 1
				$posInSinglePlayer2 = GetPositionInSinglePlayer()
				If $Counter = 15 Or $posInSinglePlayer2 = "FIRST" Then ExitLoop
			WEnd
			If $Counter = 15 Or $posInSinglePlayer2 And $rIsGoblinPicnicFound = False Then Return False
...
Func IsGoblinPicnicFound()
	...
	Local $result = multiMatchesPixelOnly($directory, 0, "FV", "", "", 0, 1000, 554, 120, 639, $GAME_HEIGHT)
