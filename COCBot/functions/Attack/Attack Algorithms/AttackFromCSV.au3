...
Func ParseAndMakeDropLines($MAINSIDE)
	...
	;Local $result[1]		; Only For ImgLoc RedLine Detection...
	Local $result
	...
	Else
		$result[0] = GetImgLoc2MBR()	; Only For ImgLoc RedLine Detection...
		;$result = DllCall($hFuncLib, "str", "getRedArea", "ptr", $hHBitmap2, "int", $xSkip, "int", $ySkip, "int", $colorVariation)
		$LastRedLines = $result[0]
	EndIf
	If $debugsetlog Then Setlog("Debug: Redline chosen")
	;Setlog(" »» NEW REDlines Imgloc: " & $result[0])
	Setlog("> NEW REDlines Imgloc in  " & Round(TimerDiff($hTimer) / 1000, 2) & " seconds", $COLOR_BLUE)	; Only For ImgLoc RedLine Detection...
