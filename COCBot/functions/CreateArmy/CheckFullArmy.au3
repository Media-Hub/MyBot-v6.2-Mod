...
Func CheckOverviewFullArmy($bOpenArmyWindow = False, $bCloseArmyWindow = False)
	...
	If $bOpenArmyWindow = True Then
		...
		While Not _ColorCheck(_GetPixelColor($btnpos[0][0], $btnpos[0][1], True), Hex(0xE8E8E0, 6), 20)
			If $debugsetlogTrain = 1 Then Setlog("OverView TabColor=" & _GetPixelColor($btnpos[0][0], $btnpos[0][1], True), $COLOR_DEBUG) ;Debug
	...
	If $debugsetlogTrain = 1 Then Setlog("Checking Overview for full army [!] " & $Pixel & ", " & _GetPixelColor(128, 176, True), $COLOR_DEBUG) ;Debug
	If $Pixel Then
		$fullArmy = True
	Else
		$fullArmy = False
	...
	If $debugSetlog = 1 Then Setlog("Can Request CC: " & $canRequestCC, $COLOR_DEBUG) ;Debug
...
Func CheckFullBarrack()
	...
	If $debugsetlogTrain = 1 Then Setlog("Check Barrack Full color : " & _GetPixelColor($aBarrackFull[0], $aBarrackFull[1], True) & " Expected if Full : " & Hex($aBarrackFull[2], 6), $COLOR_DEBUG) ;Debug
	If $debugsetlogTrain = 1 Then Setlog("Checking for Full Normal or Dark Barrack [!]" & $Pixel, $COLOR_DEBUG) ;Debug
