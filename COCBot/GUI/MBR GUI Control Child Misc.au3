...
Func btnLocateAcc7()
	LocateAcc(7)
EndFunc   ;==>btnLocateAcc7
Func btnLocateAcc8()
	LocateAcc(8)
EndFunc   ;==>btnLocateAcc8
...
Func LocateAcc($AccNo)
	...
	While 1
		...
		$stext = $sErrorText & @CRLF & "Click OK then click on your Account No. " & $AccNo & @CRLF & @CRLF & _
				"Do not move mouse quickly after clicking location" & @CRLF & @CRLF & "Please note that you have only 1 chance to click" & @CRLF
		$MsgBox = _ExtMsgBox(0, "Ok|Cancel", "Locate CoC Account No. " & $AccNo, $stext, 15, $frmBot)
		If $MsgBox = 1 Then
			WinGetAndroidHandle()
			Local $aPos = FindPos()
