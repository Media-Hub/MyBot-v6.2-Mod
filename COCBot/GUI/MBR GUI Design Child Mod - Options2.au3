...
$hGUI_ModOptions2_TAB_ITEM1 = GUICtrlCreateTabItem(GetTranslated(655,1, "Multi Accounts Option"))
	...
	$grpProfiles = GUICtrlCreateGroup(GetTranslated(655,2, "Switch Profiles"), $x - 25, $y - 25, 427, 85)
		$y -= 0
		...
		$txtVillageName = GUICtrlCreateInput("MyVillage", $x - 15, $y, 120, 20, BitOR($SS_CENTER, $ES_AUTOHSCROLL))
	...
	; Defining botting type of eachh profile - SwitchAcc - DEMEN
	    $lblProfile = GUICtrlCreateLabel(GetTranslated(655,13, "Profile Type:"), $x + 235, $y - 5, -1, -1)
			$txtTip = GetTranslated(655,14, "Choosing type for this Profile") & @CRLF & GetTranslated(655,15, "Active Profile for botting") & @CRLF & GetTranslated(655,16, "Donate Profile for donating only") & @CRLF & GetTranslated(655,17, "Idle Profile for staying inactive")
			GUICtrlSetTip(-1, $txtTip)

	    $radActiveProfile= GUICtrlCreateRadio(GetTranslated(655,18, "Active"), $x + 235, $y + 13, -1, 16)
			GUICtrlSetTip(-1, GetTranslated(655,19, "Set as Active Profile for training troops & attacking"))
			GUICtrlSetState(-1, $GUI_CHECKED)
			GUICtrlSetOnEvent(-1, "radProfileType")

		$radDonateProfile = GUICtrlCreateRadio(GetTranslated(655,20, "Donate"), $x + 295, $y + 13, -1, 16)
			GUICtrlSetTip(-1, GetTranslated(655,21, "Set as Donating Profile for training troops & donating only"))
			GUICtrlSetOnEvent(-1, "radProfileType")

		$radIdleProfile = GUICtrlCreateRadio(GetTranslated(655,22, "Idle"), $x + 355, $y + 13, -1, 16)
			GUICtrlSetTip(-1, GetTranslated(655,23, "Set as Idle Profile. The Bot will ignore this Profile"))
			GUICtrlSetOnEvent(-1, "radProfileType")

	    $lblMatchProfileAcc = GUICtrlCreateLabel(GetTranslated(655,24, "Matching Acc. No."), $x + 235, $y + 37, -1, 16)
			$txtTip = GetTranslated(655,25, "Select the index of CoC Account to match with this Profile")
			GUICtrlSetTip(-1, $txtTip)
	...
	; SwitchAcc - DEMEN
	Local $x = 25, $y = 130
	$grpSwitchAcc = GUICtrlCreateGroup(GetTranslated(655,26, "Switch Account Mode"), $x - 20, $y - 20, 210, 150)

		$chkSwitchAcc = GUICtrlCreateCheckbox(GetTranslated(655,27, "Enable Switch Account"), $x - 10, $y, -1, -1)
			$txtTip = GetTranslated(655,28, "Switch to another account & profile when troop training time is >= 3 minutes") & @CRLF & GetTranslated(655,29, "This function supports maximum 6 CoC accounts & 6 Bot profiles") & @CRLF & GetTranslated(655,30, "Make sure to create sufficient Profiles equal to number of CoC Accounts, and align the index of accounts order with profiles order")
			GUICtrlSetTip(-1, $txtTip)
			GUICtrlSetOnEvent(-1, "chkSwitchAcc")

		$lblTotalAccount = GUICtrlCreateLabel(GetTranslated(655,31, "Total CoC Acc:"), $x + 15, $y + 29, -1, -1)
			$txtTip = GetTranslated(655,32, "Choose number of CoC Accounts pre-logged")
			GUICtrlSetState(-1, $GUI_DISABLE)
		...
		$radSmartSwitch = GUICtrlCreateRadio(GetTranslated(655,33, "Smart switch"), $x + 15 , $y + 55, -1, 16)
			GUICtrlSetTip(-1, GetTranslated(655,34, "Switch to account with the shortest remain training time"))
			GUICtrlSetState(-1, $GUI_CHECKED)
			GUICtrlSetState(-1, $GUI_DISABLE)

		$radNormalSwitch = GUICtrlCreateRadio(GetTranslated(655,35, "Normal switch"), $x + 100, $y + 55, -1, 16)
			GUICtrlSetTip(-1, GetTranslated(655,36, "Switching accounts continously"))
			GUICtrlSetState(-1, $GUI_DISABLE)
			GUICtrlSetOnEvent(-1, "radNormalSwitch")

		$y += 80

		$chkUseTrainingClose = GUICtrlCreateCheckbox(GetTranslated(655,37, "Combo Sleep after Switch Account"), $x, $y, -1, -1)
			$txtTip = GetTranslated(655,38, "Close CoC combo with Switch Account when there is more than 3 mins remaining on training time of all accounts.")
			GUICtrlSetTip(-1, $txtTip)
			GUICtrlSetOnEvent(-1, "chkUseTrainingClose")

		$radCloseCoC= GUICtrlCreateRadio(GetTranslated(655,39, "Close CoC"), $x + 15 , $y + 30, -1, 16)
			GUICtrlSetState(-1, $GUI_CHECKED)

		$radCloseAndroid = GUICtrlCreateRadio(GetTranslated(655,40, "Close Android"), $x + 100, $y + 30, -1, 16)
	GUICtrlCreateGroup("", -99, -99, 1, 1)

	; Profiles & Account matching

    Local $x = 238, $y = 130
	  $grpSwitchAccMapping = GUICtrlCreateGroup(GetTranslated(655,41, "Profiles"), $x - 20, $y - 20, 215, 250)

		$btnUpdateProfiles = GUICtrlCreateButton(GetTranslated(655,42, "Update Profiles/ Acc matching"), $x, $y - 5 , 170, 25)
	...
	Local $x = 25, $y = 280
	$grpRestartAndroid = GUICtrlCreateGroup(GetTranslated(655,43, "Restart Android"), $x - 20, $y - 20, 210, 100)

	    $chkRestartAndroid = GUICtrlCreateCheckbox(GetTranslated(655,44, "Restart Android every") & ":", $x, $y, -1, -1)
			$txtTip = GetTranslated(655,45, "Restart Android after long searches or after getting stuck at opening train window")
		...
		$txtRestartAndroidSearchLimit = GUICtrlCreateInput("200", $x + 20, $y + 30, 30, 18, BitOR($GUI_SS_DEFAULT_INPUT, $ES_CENTER, $ES_NUMBER))
			...
			GUICtrlSetLimit(-1, 4)
			GUICtrlSetState(-1, $GUI_DISABLE)
		$lblRestartAndroidSearchLimit = GUICtrlCreateLabel(GetTranslated(655,46, "Search(es)"), $x + 57, $y + 32, -1, -1)

		$txtRestartAndroidTrainError = GUICtrlCreateInput("10", $x + 20, $y + 55, 30, 18, BitOR($GUI_SS_DEFAULT_INPUT, $ES_CENTER, $ES_NUMBER))
			GUICtrlSetTip(-1, $txtTip)
			GUICtrlSetLimit(-1, 4)
			GUICtrlSetState(-1, $GUI_DISABLE)
		$lblRestartAndroidTrainError = GUICtrlCreateLabel(GetTranslated(655,47, "times error at Train Window"), $x + 57, $y + 57, -1, -1)
	GUICtrlCreateGroup("", -99, -99, 1, 1)

$hGUI_ModOptions2_TAB_ITEM2 = GUICtrlCreateTabItem(GetTranslated(655,48, "Switch Profile Option"))
	; Switch Profile
	Local $xStart = 0, $yStart = 0
	Local $x = $xStart + 30, $y = $yStart + 50
	$grpGoldSwitch = GUICtrlCreateGroup(GetTranslated(655,49, "Gold Switch Profile Conditions"), $x - 20, $y - 20, 420, 75) ;Gold Switch
		$chkGoldSwitchMax = GUICtrlCreateCheckbox(GetTranslated(655,50, "Switch To"), $x - 10, $y - 5, -1, -1)
			$txtTip = GetTranslated(655,51, "Enable this to switch profiles when gold is above amount.")
			_GUICtrlSetTip(-1, $txtTip)
		$cmbGoldMaxProfile = GUICtrlCreateCombo("", $x + 60, $y - 5, 75, -1, BitOR($CBS_DROPDOWNLIST, $CBS_AUTOHSCROLL))
			$txtTip = GetTranslated(655,53, "Select which profile to be switched to when conditions met")
			_GUICtrlSetTip(-1, $txtTip)
		$lblGoldMax = GUICtrlCreateLabel(GetTranslated(655,55, "When Gold is Above"), $x + 145, $y, -1, -1)
		$txtMaxGoldAmount = GUICtrlCreateInput("6000000", $x + 275, $y - 5, 50, 18, BitOR($GUI_SS_DEFAULT_INPUT, $ES_CENTER, $ES_NUMBER))
			$txtTip = GetTranslated(655,54, "Set the amount of Gold to trigger switching Profile.")
			_GUICtrlSetTip(-1, $txtTip)
			GUICtrlSetLimit(-1, 7)

	$y += 30
		$chkGoldSwitchMin = GUICtrlCreateCheckbox(GetTranslated(655,33, "Switch To"), $x - 10, $y - 5, -1, -1)
			$txtTip = GetTranslated(655,52, "Enable this to switch profiles when gold is below amount.")
			_GUICtrlSetTip(-1, $txtTip)
		$cmbGoldMinProfile = GUICtrlCreateCombo("", $x + 60, $y - 5, 75, -1, BitOR($CBS_DROPDOWNLIST, $CBS_AUTOHSCROLL))
			$txtTip = GetTranslated(655,53, "Select which profile to be switched to when conditions met")
			_GUICtrlSetTip(-1, $txtTip)
		$lblGoldMin = GUICtrlCreateLabel(GetTranslated(655,56, "When Gold is Below"), $x + 145, $y, -1, -1)
	...
	GUICtrlCreateGroup("", -99, -99, 1, 1)
	$y += 48
	$grpElixirSwitch = GUICtrlCreateGroup(GetTranslated(655,57, "Elixir Switch Profile Conditions"), $x - 20, $y - 20, 420, 75) ; Elixir Switch
		$chkElixirSwitchMax = GUICtrlCreateCheckbox(GetTranslated(655,50, "Switch To"), $x - 10, $y - 5, -1, -1)
			$txtTip = GetTranslated(655,58, "Enable this to switch profiles when Elixir is above amount.")
			_GUICtrlSetTip(-1, $txtTip)

		$cmbElixirMaxProfile = GUICtrlCreateCombo("", $x + 60, $y - 5, 75, -1, BitOR($CBS_DROPDOWNLIST, $CBS_AUTOHSCROLL))
			$txtTip = GetTranslated(655,53, "Select which profile to be switched to when conditions met")
			_GUICtrlSetTip(-1, $txtTip)
		$lblElixirMax = GUICtrlCreateLabel(GetTranslated(655,61, "When Elixir is Above"), $x + 145, $y, -1, -1)
		$txtMaxElixirAmount = GUICtrlCreateInput("6000000", $x + 275, $y - 5, 50, 18, BitOR($GUI_SS_DEFAULT_INPUT, $ES_CENTER, $ES_NUMBER))
			$txtTip = GetTranslated(655,60, "Set the amount of Elixir to trigger switching Profile.")
			_GUICtrlSetTip(-1, $txtTip)
			GUICtrlSetLimit(-1, 7)
	$y += 30
		$chkElixirSwitchMin = GUICtrlCreateCheckbox(GetTranslated(655,50, "Switch To"), $x - 10, $y - 5, -1, -1)
			$txtTip = GetTranslated(655,59, "Enable this to switch profiles when Elixir is below amount.")
			_GUICtrlSetTip(-1, $txtTip)
		$cmbElixirMinProfile = GUICtrlCreateCombo("", $x + 60, $y - 5, 75, -1, BitOR($CBS_DROPDOWNLIST, $CBS_AUTOHSCROLL))
			$txtTip = GetTranslated(655,53, "Select which profile to be switched to when conditions met")
			_GUICtrlSetTip(-1, $txtTip)
		$lblElixirMin = GUICtrlCreateLabel(GetTranslated(655,62, "When Elixir is Below"), $x + 145, $y, -1, -1)
	...
	GUICtrlCreateGroup("", -99, -99, 1, 1)
	$y += 48
	$grpDESwitch = GUICtrlCreateGroup(GetTranslated(655,63, "Dark Elixir Switch Profile Conditions"), $x - 20, $y - 20, 420, 75) ;DE Switch
		$chkDESwitchMax = GUICtrlCreateCheckbox(GetTranslated(655,33, "Switch To"), $x - 10, $y - 5, -1, -1)
			$txtTip = GetTranslated(655,64, "Enable this to switch profiles when Dark Elixir is above amount.")
			_GUICtrlSetTip(-1, $txtTip)
		$cmbDEMaxProfile = GUICtrlCreateCombo("", $x + 60, $y - 5, 75, -1, BitOR($CBS_DROPDOWNLIST, $CBS_AUTOHSCROLL))
			$txtTip = GetTranslated(655,53, "Select which profile to be switched to when conditions met")
			_GUICtrlSetTip(-1, $txtTip)
		$lblDEMax = GUICtrlCreateLabel(GetTranslated(655,67, "When Dark Elixir is Above"), $x + 145, $y, -1, -1)
		$txtMaxDEAmount = GUICtrlCreateInput("200000", $x + 275, $y - 5, 50, 18, BitOR($GUI_SS_DEFAULT_INPUT, $ES_CENTER, $ES_NUMBER))
			$txtTip = GetTranslated(655,66, "Set the amount of Dark Elixir to trigger switching Profile.")
			_GUICtrlSetTip(-1, $txtTip)
			GUICtrlSetLimit(-1, 6)
	$y += 30
		$chkDESwitchMin = GUICtrlCreateCheckbox(GetTranslated(655,50, "Switch To"), $x - 10, $y - 5, -1, -1)
			$txtTip = GetTranslated(655,65, "Enable this to switch profiles when Dark Elixir is below amount.")
			_GUICtrlSetTip(-1, $txtTip)
		$cmbDEMinProfile = GUICtrlCreateCombo("", $x + 60, $y - 5, 75, -1, BitOR($CBS_DROPDOWNLIST, $CBS_AUTOHSCROLL))
			$txtTip = GetTranslated(655,53, "Select which profile to be switched to when conditions met")
			_GUICtrlSetTip(-1, $txtTip)
		$lblDEMin = GUICtrlCreateLabel(GetTranslated(655,68, "When Dark Elixir is Below"), $x + 145, $y, -1, -1)
	...
	GUICtrlCreateGroup("", -99, -99, 1, 1)
	$y += 48
	$grpTrophySwitch = GUICtrlCreateGroup(GetTranslated(655,69, "Trophy Switch Profile Conditions"), $x - 20, $y - 20, 420, 75) ; Trophy Switch
		$chkTrophySwitchMax = GUICtrlCreateCheckbox(GetTranslated(655,33, "Switch To"), $x - 10, $y - 5, -1, -1)
			$txtTip = GetTranslated(655,70, "Enable this to switch profiles when Trophies are above amount.")
			_GUICtrlSetTip(-1, $txtTip)
		$cmbTrophyMaxProfile = GUICtrlCreateCombo("", $x + 60, $y - 5, 75, -1, BitOR($CBS_DROPDOWNLIST, $CBS_AUTOHSCROLL))
			$txtTip = GetTranslated(655,53, "Select which profile to be switched to when conditions met")
			_GUICtrlSetTip(-1, $txtTip)
		$lblTrophyMax = GUICtrlCreateLabel(GetTranslated(655,73, "When Trophies are Above"), $x + 145, $y, -1, -1)
		$txtMaxTrophyAmount = GUICtrlCreateInput("3000", $x + 275, $y - 5, 50, 18, BitOR($GUI_SS_DEFAULT_INPUT, $ES_CENTER, $ES_NUMBER))
			$txtTip = GetTranslated(655,72, "Set the amount of Trophies to trigger switching Profile.")
			_GUICtrlSetTip(-1, $txtTip)
			GUICtrlSetLimit(-1, 4)
	$y += 30
		$chkTrophySwitchMin = GUICtrlCreateCheckbox(GetTranslated(655,50, "Switch To"), $x - 10, $y - 5, -1, -1)
			$txtTip = GetTranslated(655,71, "Enable this to switch profiles when Trophies are below amount.")
			_GUICtrlSetTip(-1, $txtTip)
		$cmbTrophyMinProfile = GUICtrlCreateCombo("", $x + 60, $y - 5, 75, -1, BitOR($CBS_DROPDOWNLIST, $CBS_AUTOHSCROLL))
			$txtTip = GetTranslated(655,53, "Select which profile to be switched to when conditions met")
			_GUICtrlSetTip(-1, $txtTip)
		$lblTrophyMin = GUICtrlCreateLabel(GetTranslated(655,74, "When Trophies are Below"), $x + 145, $y, -1, -1)
